...
begin
  ...
  unless vm.retired?
    $evm.log(:error, "The VM, #{vm.name}, is not retired. The VM must be retired before moving it to the graveyard")
    exit MIQ_ABORT
  end
  
  if vm.power_state == "on"
    $evm.log(:warn, "The VM, #{vm.name}, is still powered on. Powering it off.")
    vm.stop
  end
  ...
  datastore_name = $evm.object['graveyard_datastore']
  datastore = $evm.vmdb('storage').find_by_name(datastore_name)
  datastore_ems_ref = datastore.ems_ref  	
