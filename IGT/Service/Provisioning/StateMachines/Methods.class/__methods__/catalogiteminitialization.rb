...
#### Modified for IGT ####
def add_custom_tags(dialog_options_hash, dialog_tags_hash)
  $evm.log(:info, "dialog_options_hash: #{dialog_options_hash.inspect}")
  
  folder_path = dialog_options_hash.fetch(:folder_path_product, nil)
  $evm.log(:info, "folder_path: #{folder_path}")
  if folder_path.nil?
    $evm.log(:error, "folder_path was nil. Aborting provision.")
    exit MIQ_ABORT
  end
  
  split_folder_path = folder_path.split(" ")
  product = split_folder_path.last
  $evm.log(:info, "product: #{product}")
  if product.nil?
    $evm.log(:error, "product was nil. Aborting provision")
    exit MIQ_ABORT
  end
  
  folder_path = dialog_options_hash.fetch(:folder_path_site, nil)
  $evm.log(:info, "folder_path: #{folder_path}")
  if folder_path.nil?
    $evm.log(:error, "folder_path was nil. Aborting provision")
    exit MIQ_ABORT
  ...
  split_folder_path = folder_path.split("/")
  site = split_folder_path.last
  $evm.log(:info, "site: #{site}")
  if site.nil?
    $evm.log(:error, "site was nil. Aborting provision")
    exit MIQ_ABORT
  end
  
  dialog_tags_hash["site"] = site
  dialog_tags_hash["product"] = product
  $evm.log(:info, "dialog_tags_hash: #{dialog_tags_hash.inspect}")
end

def set_retirement_date(dialog_options_hash, prov)
  $evm.log(:info, "entering set_retirement_date")
  $evm.log(:info, "prov: #{prov.inspect}")
  service =  prov.miq_request_task.destination
  retirement_date_type = dialog_options_hash.fetch(:retirement_date_type, nil)
  if retirement_date_type == "end_date"
    retirement_date = dialog_options_hash.fetch(:retirement_date, nil)
    retirement_date = Date.parse(retirement_date)
  else
  	retirement_date = (dialog_options_hash.fetch(:relative_retirement_date, nil)).to_i
    retirement_date = Date.today + retirement_date.month
  end
  
  $evm.log(:info, "retirement_date: #{retirement_date}")
  
  # we want retirement to be handled by the bundle service
  if service.has_parent
    service = service.parent_service
    $evm.log(:info, "service has parent, setting retirement date on parent instead")
  end
  
  #because we're letting children services set the parent services retirement dates
  #we want to make sure a child hasn't already set the date, otherwise we'll end up with
  #several retirment events being triggered
  if service.retires_on.nil? && service.retirement_warn.nil?
...
def set_vm_name(dialog_options_hash, prov)
  #The name is set by /Infrastructure/VM/Provisioning/Naming/default prior to this step
  new_vm_name = prov.get_option(:vm_target_name) || prov.get_option(:vm_target_hostname) || prov.get_option(:vm_name) || prov.get_option(:linux_host_name)
...
def pass_dialog_values_to_provision_task(provision_task, dialog_options_hash, dialog_tags_hash)
  provision_task.miq_request_tasks.each do |prov|
    ...
    set_retirement_date(dialog_options_hash, prov)
...
def parsed_dialog_information
  ...
  #### Modified for IGT ####
  add_custom_tags(merged_options_hash, merged_tags_hash)
  #### Modified for IGT ####
