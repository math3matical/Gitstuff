...
tags = prov.get_tags
#grab os_type off of the template
tags[:os_type] = vm.tags("os_type")
#remove environment, site, and product from the tags hash, because we don't want to use them for placement
tags.delete(:environment)
tags.delete(:site)
tags.delete(:product)

$evm.log(:info, "placement_tags: #{tags.inspect}")
...
prov.eligible_hosts.each do |h|
  ...
  $evm.log(:info, "writable storages:")
  h.writable_storages.each {|storage| $evm.log(:info, "#{storage.name}")}
...
if storage
  ...
  $evm.log(:error, "No storage was found with the appropriate placement tags. Aborting service provision!")
  exit MIQ_ABORT
end
